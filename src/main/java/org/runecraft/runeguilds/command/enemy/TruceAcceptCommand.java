...
public class TruceAcceptCommand implements CommandExecutor {
    ...
    public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
        ...
        target.removeEnemy(guild, true);
        guild.removeEnemy(target, true);
        guild.getOnlineMembers().forEach(x->{
            Player p = x.getKey().getPlayer().get();
            p.sendMessage(Text.builder("*" + target.getName().toPlain() + " mão é mais um inimigo.").color(TextColors.YELLOW).build());
            p.playSound(SoundTypes.BLOCK_NOTE_BELL, p.getPosition(), 1.5f);
        });
        target.getOnlineMembers().forEach(x->{
            Player p = x.getKey().getPlayer().get();
            p.sendMessage(Text.builder("*" + guild.getName().toPlain() + " mão é mais um inimigo.").color(TextColors.YELLOW).build());
            p.playSound(SoundTypes.BLOCK_NOTE_BELL, p.getPosition(), 1.5f);
        });
        return CommandResult.builder().build();
    }
